...
import { JiraServerIntegrationForm } from "./jira-server-integration-form";

interface JiraServerIntegrationsManagerProps {
  integrations: IntegrationProps[];
  metadata?: MetaDataProps;
}

export const JiraServerIntegrationsManager = ({
  integrations,
  metadata,
}: JiraServerIntegrationsManagerProps) => {
  ...
  const handleDeleteIntegration = async (id: string) => {
    setIsDeleting(id);
    try {
      const result = await deleteIntegration(id, "jira_server");

      if (result.success) {
        toast({
          title: "Success!",
          description: "Jira Server integration deleted successfully.",
        });
      ...
      toast({
        ...
        title: "Error",
        description:
          "Failed to delete Jira Server integration. Please try again.",
      });
    } finally {
      setIsDeleting(null);
  ...
  return (
    <>
      <Modal
        ...
        title="Delete Jira Server Integration"
        description="This action cannot be undone. This will permanently delete your Jira Server integration."
      >
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold">
              Configured Jira Server Integrations
            </h3>
        ...
        {isOperationLoading ? (
          <IntegrationSkeleton
            ...
            title="Jira Server"
            subtitle="Create and manage security issues in a self-hosted Jira."
          />
        ) : integrations.length > 0 ? (
            ...
            {integrations.map((integration) => (
              <Card key={integration.id} variant="base">
                <CardHeader>
                  <IntegrationCardHeader
                    icon={<JiraIcon size={32} />}
                    title={`${integration.attributes.configuration.base_url}`}
                    connectionStatus={{
